...
struct Point   //定义一个名为Student的结构体
{
   ...
   string present; //介绍语
	...
	struct Point m_point[5]={{2.48,0.13,0.03,0.99,"北京","  北京，中国的首都"},{2.48,1.077,-0.026,1.000,"广州","广广广州，自古以来都是中国的商都"},{2.48,2.120,-0.032,0.999,"吉林","吉吉吉林，位于中国的东北是人参之都"},{1.026,1.109,-0.012,1.000,"深圳","深深深圳，中国的科创中心"},{1.073,2.120,0.013,1.000,"上海","上上上海，中国的经济中心"}};
        /*
...
int main(int argc,char **argv){
    ...
    while(ros::ok()){
        ...
        if(text.find("参观一圈") != string::npos){ //识别到“导航”关键词
            ...
            goal1.target_pose.pose.position.x = 2.562;//目标点x 
            goal1.target_pose.pose.position.y = 0.023;//目标点y  
            goal1.target_pose.pose.orientation.z =0.106;
            goal1.target_pose.pose.orientation.w = 0.994;//目标点姿态四元数表示
            ...
            goal2.target_pose.pose.position.x = 2.567;//目标点x  
            goal2.target_pose.pose.position.y =1.054; //目标点y  
            goal2.target_pose.pose.orientation.z =  -0.005;  
            ...
            move_base_msgs::MoveBaseGoal goal3;//定义第三个目标点 ，吉林馆 2.563,2.013,-0.026,1.000
            goal3.target_pose.header.frame_id = "map";  
            goal3.target_pose.header.stamp = ros::Time::now();  
            goal3.target_pose.pose.position.x = 2.563;//目标点x  
            goal3.target_pose.pose.position.y = 2.013;    //目标点y  
            goal3.target_pose.pose.orientation.z = -0.026;  
            goal3.target_pose.pose.orientation.w =1.000;//目标点姿态四元数表示
            ...
            goal4.target_pose.pose.position.x =1.040;//目标点x  
            goal4.target_pose.pose.position.y =  1.102;    //目标点y  
            goal4.target_pose.pose.orientation.z = -0.063;  
            goal4.target_pose.pose.orientation.w =0.998;//目标点姿态四元数表示
            ...
            goal5.target_pose.pose.position.x =1.134;//目标点x  
            goal5.target_pose.pose.position.y = 2.129;    //目标点y  
            goal5.target_pose.pose.orientation.z =-0.063;  
            goal5.target_pose.pose.orientation.w = 0.998;//目标点姿态四元数表示
            ...
            goal6.target_pose.pose.position.x =-0.006;//目标点x  
            goal6.target_pose.pose.position.y = -0.069;    //目标点y  
            goal6.target_pose.pose.orientation.z =0.018;
