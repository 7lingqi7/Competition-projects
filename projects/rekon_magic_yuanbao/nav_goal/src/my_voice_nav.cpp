...
struct Point   //定义一个名为Student的结构体
{
	...
	struct Point m_point[5]={{2.512,0.015,-0.039,0.999,"北京","北北北京，中国的首都"},{2.567,1.054,-0.005,1.000,"广州","广广广州，自古以来都是中国的商都"},{2.563,2.013,-0.026,1.000,"吉林","吉吉吉林，位于中国的东北是人参之都"},{1.040,1.102,-0.063,0.998,"深圳","深深深圳，中国的科创中心"},{1.134,2.129,-0.063,0.998,"上海","上上上海，中国的经济中心"}};
        /*
...
int main(int argc,char **argv){
    ...
    ros::ServiceClient relative_move_client = nh.serviceClient<relative_move::SetRelativeMove>("relative_move");//定义相对运动客户端
    ros::ServiceClient ar_track_client = nh.serviceClient<ar_pose::Track>("track");//定义ar码跟踪客户端
        ...
        if(text.find("要到") != string::npos){ //识别到“导航”关键词
            for(int i=0;i<5;i++){ //遍历所有参数
                if(text.find(m_point[i].name.c_str()) != string::npos){ //查找所有导航点是否有匹配的导航点
                    ...
                    // 导航到ar码附近
		            move_base_msgs::MoveBaseGoal goal7;//定义第七个目标点 充电点
                    goal7.target_pose.header.frame_id = "map";  
                    goal7.target_pose.header.stamp = ros::Time::now();  
                    goal7.target_pose.pose.position.x =0.451;//目标点x  
                    goal7.target_pose.pose.position.y = 1.919;    //目标点y  
                    goal7.target_pose.pose.orientation.z =-0.734;
                    goal7.target_pose.pose.orientation.w = 0.679;//目标点姿态四元数表示
                    ac.sendGoal(goal7);  
